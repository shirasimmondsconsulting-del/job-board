...
import { Search, Filter, X, RefreshCcw, MapPin, Briefcase, SlidersHorizontal, ChevronDown, Plus } from 'lucide-react'
...
function Jobs() {
  const { filters, setFilters, getFilteredJobs, clearFilters, fetchJobs } = useJobs()
  const { user } = useAuth()
  ...
  const timelineOptions = [
    { value: 'all', label: 'All Timelines', emoji: '📅' },
  ...
  const locations = [
    'Tel Aviv', 'Jerusalem', 'Haifa', 'Sharon', 'Shomron',
    'Gush Etzion', 'North', 'South', 'Petah Tikva'
  ]
  ...
  const activeFilterCount = [filters.location, filters.industry, filters.hebrewLevel, filters.remote, filters.search].filter(Boolean).length
        ...
        <div className="container-custom relative">
          <div className="max-w-3xl mx-auto text-center">
            ...
            >
              ...
              <span className="text-sm font-medium text-white">{filteredJobs.length} opportunities available</span>
            </motion.div>
            ...
            >
              Every position listed here welcomes Olim. Filter by your timeline and find your next opportunity.
            </motion.p>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 lg:py-12">
        <div className="container-custom">
          ...
          >
                ...
                <button
                  onClick={() => {
                    if (!user || user.userType !== 'employer') {
                      window.location.href = '/login?next=/jobs&msg=employer_required';
                    } else {
            ...
            {/* Timeline Tabs */}
            <div className="mb-6">
              <label className="text-sm font-semibold text-gray-700 mb-3 block">When are you arriving in Israel?</label>
              <div className="flex flex-wrap gap-2">
                {timelineOptions.map(option => (
                  <button
                    key={option.value}
                    onClick={() => handleFilterChange('timeline', option.value)}
                    className={`inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${filters.timeline === option.value
                  ...
                  <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6 pt-6 border-t border-gray-100">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => handleFilterChange('hebrewLevel', e.target.value)}
                        className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 text-gray-900 text-sm appearance-none bg-white"
                      >
                    ...
                    <div>
                      <label className="text-sm font-medium text-gray-700 mb-2 block">Remote Work</label>
                      <select
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {filters.location && (
                    <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary-50 text-primary-700 rounded-lg text-xs font-semibold border border-primary-100">
                      ...
                      {filters.location}
                      <button onClick={() => handleFilterChange('location', '')} className="hover:text-primary-900">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  {filters.industry && (
                    <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary-50 text-primary-700 rounded-lg text-xs font-semibold border border-primary-100">
                      {filters.industry}
                      <button onClick={() => handleFilterChange('industry', '')} className="hover:text-primary-900">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  {filters.hebrewLevel && (
                    <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary-50 text-primary-700 rounded-lg text-xs font-semibold border border-primary-100">
                      ...
                      <button onClick={() => handleFilterChange('hebrewLevel', '')} className="hover:text-primary-900">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
                  {filters.remote && (
                    <span className="inline-flex items-center gap-2 px-3 py-1.5 bg-primary-50 text-primary-700 rounded-lg text-xs font-semibold border border-primary-100">
                      ...
                      <button onClick={() => handleFilterChange('remote', '')} className="hover:text-primary-900">
                        <X className="w-3 h-3" />
                      </button>
                    </span>
                  )}
          ...
          <div className="flex items-center justify-between mb-6">
            <p className="text-gray-600">
              Showing <span className="font-bold text-gray-900">{filteredJobs.length}</span> {filteredJobs.length === 1 ? 'job' : 'jobs'}
            </p>
            <div className="text-sm text-gray-500">
              Sorted by: <span className="font-medium text-gray-700">Most Recent</span>
            </div>
          </div>
          ...
          {filteredJobs.length > 0 ? (
            ...
            >
              ...
              <p className="text-gray-600 mb-6 max-w-sm mx-auto">Try adjusting your filters or search terms to see more results</p>
              <button
                onClick={clearFilters}
